...
df_acc_excel <- df_excel %>% select("Account Id":"Industry", "Longitude", "Latitude")
df_opp_excel <- df_excel %>% select("Opportunity ID":"Opportunity Quantity", "Created Date", "Close Date")
...
df_csv_acc$`Longitude` <- as.double(df_csv_acc$`Longitude`)
df_csv_acc$`Latitude` <- as.double(df_csv_acc$`Latitude`)
...
ornek_liste <- c("Apparel", "Banking", "Biotechnology", "Chemicals", "Communications", "Construction", "Consulting", "Education", "Electronics", "Energy", "Engineering", "Entertainment", "Environmental", "Finance", "Government", "Healthcare", "Hospitality", "Insurance", "Machinery", "Manufacturing", "Media", "Not For Profit", "Recreation", "Retail", "Shipping", "Technology", "Telecommunications", "Transportation", "Utilities", "Other")

en_yuksek_benzerlik <- function(deger, ornek_liste, esik_orani) {
  benzerlik_oranlari <- 1 - stringdist::stringdistmatrix(deger, ornek_liste, method = "jw")
  max_index <- which.max(benzerlik_oranlari)
  max_oran <- benzerlik_oranlari[max_index]
  
  if (max_oran > esik_orani) {
    return(ornek_liste[max_index])
  } else {
    return("Other")
  }
}

df_csv_acc_updated$`Industry` <- sapply(df_csv_acc_updated$`Industry`, en_yuksek_benzerlik, ornek_liste, esik_orani)

df_csv_acc_updated$`Type` <- "Other"

temp <- df_csv_acc_updated
df_csv_acc_updated <- df_csv_acc_updated %>%
  group_by(Name) %>%
  summarize_all(funs(first))

df_csv_acc_updated_noNA <- df_csv_acc_updated %>% mutate_all(~ ifelse(is.na(.), "", .))
df_csv_acc_updated_noNA <- df_csv_acc_updated_noNA %>%
  mutate(Deleted = ifelse(Deleted == "true", 1, 0))
df_csv_acc_updated_noNA$`Date Created` <- as.POSIXct(df_csv_acc_updated_noNA$`Date Created`, origin="1970-01-01", tz="UTC")
df_csv_acc_updated_noNA$`Date Created` <- format(df_csv_acc_updated_noNA$`Date Created`, "%Y-%m-%d %H:%M:%S")
write.csv(df_csv_acc_updated_noNA, "Accounts_updated_noNA.csv", row.names = FALSE)
...
df_csv_opp$`Date Created` <- as_datetime(df_csv_opp$`Date Created`)
df_csv_opp$`Expected Close Date` <- as_datetime(df_csv_opp$`Expected Close Date`)
...
df_csv_opp_updated$`Account Name` <- temp$`Name`

df_csv_opp_updated_noNA <- df_csv_opp_updated %>% mutate_all(~ ifelse(is.na(.), "", .))
df_csv_opp_updated_noNA$`Date Created` <- as.POSIXct(df_csv_opp_updated_noNA$`Date Created`, origin="1970-01-01", tz="UTC")
df_csv_opp_updated_noNA$`Date Created` <- format(df_csv_opp_updated_noNA$`Date Created`, "%m/%d/%Y %H:%M")
df_csv_opp_updated_noNA$`Expected Close Date` <- as.POSIXct(df_csv_opp_updated_noNA$`Expected Close Date`, format="%m/%d/%Y %H:%M", tz="UTC")
df_csv_opp_updated_noNA$`Expected Close Date` <- format(df_csv_opp_updated_noNA$`Expected Close Date`, "%m/%d/%Y")
write.csv(df_csv_opp_updated_noNA, "Opportunities_updated_noNA.csv", row.names = FALSE)
...
Account veri setimizin 696, Opportunities veri setimizin 3215 tane verisi bulunmaktadır.

### Account veri seti

```{r}

datatable(df_csv_acc_updated_noNA,, colnames = c('#' = 1), class = 'cell-border stripe',  style = "bootstrap5", options = list(
  server = TRUE,
  processing = TRUE,
  searching = FALSE,
  scrollX = TRUE,
...
```{r}
datatable(df_csv_opp_updated_noNA, colnames = c('#' = 1), class = 'cell-border stripe', style = "bootstrap5", options = list(
  server = TRUE,
  processing = TRUE,
  searching = FALSE,
  scrollX = TRUE,
...
Veri setleri incelendiğinde 39 farklı sektörde hesap bulunduğu görülüyor. Bu sektörlerden en çok tercih edilenleri teknoloji ve sağlık hizmetleridir.
...
Video adresi: [https://www.youtube.com/watch?v=Jp5VsvYLxSc](https://www.youtube.com/watch?v=Jp5VsvYLxSc)